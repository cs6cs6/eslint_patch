 ...
 * @param {string} cwd the current workding directory
...
class LintResultCache {
     ...
     * @param {boolean} shareableCache indicates whether to use full or partial file paths
     * @param {string} cwd the current working directory
     */
    constructor(cacheFileLocation, cacheStrategy, shareableCache, cwd) {
        ...
        debug(`shareableCache=${shareableCache} and cwd=${cwd}`);
        ...
        this.shareableCache = shareableCache;
        this.cwd = cwd;
    }

    /**
     * Depending on if the cache is supposed to be shareable, store the results in a map
     * by either a relative or a full path
     * @param {string} filePath
     * @returns {string} the cache key to use
     */
    getFileKey(filePath){
        if(!this.shareableCache) {
            return filePath;
        }
        return filePath.replace(this.cwd, ".");
    }
    ...
    getCachedLintResults(filePath, config) {
         ...
         */
        const fileDescriptor = this.fileEntryCache.getFileDescriptor(this.getFileKey(filePath));
        const hashOfConfig = hashOfConfigFor(config, this.cwd);
        const changed =
    ...
    setCachedLintResults(filePath, config, result) {
        ...
        const fileDescriptor = this.fileEntryCache.getFileDescriptor(fileKey);
        ...
        if (fileDescriptor && !fileDescriptor.notFound) {
            ...
            if(this.shareableCache && resultToSerialize.filePath ) {
                resultToSerialize.filePath = fileKey;
            }
            ...
            fileDescriptor.meta.hashOfConfig = hashOfConfigFor(config, this.cwd);
        }
    }
